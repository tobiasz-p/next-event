...
- `sync/next-event-sync` writes the JSON calendar state. The Google CLI is pluggable: `gws` is the default, `gog` is optional (`NEXT_EVENT_CLI=gws|gog|auto`, or a `cli` file in the profile directory).
